				...
				<div class="banner-textt">
					<form method="post" class="form-inline">
						<div class="form-group mb-2">
							<label for="staticEmail2">Immatricule d'Avion</label>
						</div>
						<div class="form-group mx-sm-3 mb-2">
							<input type="text" class="form-control" name="avion" placeholder="Immatricule d'Avion">
						</div>
					...
					<fieldset style="background-color: rgba(0, 0, 0 ,0.50)">
						<legend>
							<a href="index.html" class="logo">Liste des interventions par avion</a>
						</legend>
						<?php
						try {
						if(isset($_POST['avion'])){
							...
							$req = "SELECT  objet, date FROM interviention WHERE Immatricule='$avion'";
							$rep = $conn->query($req);
							if ($rep) {
								echo "<table class='table'>";
								echo "<th style='color:#586EFB;'>Objet</th>
								<th style='color:#586EFB;'>Date</th>";
								foreach ($rep as $tab) {
									echo "<tr><td style='color:white;'>$tab[objet]</td>
									<td style='color:white;'>$tab[date]</td></tr>";
								}
